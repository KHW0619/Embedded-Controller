...
// 3. Configure prescaler PSC
	float fclk = (float)fsys/(float)(psc+1);					// fclk=fsys/(psc+1);
...
// 1. Configure prescaler PSC
	float value = (float)(TIMx->ARR + 1) * duty + 1;    								// (ARR+1)*dutyRatio + 1
   
  	if(chN == 1)      { TIMx->CCR1 = value; }          //set channel      
  	if(chN == 2)      { TIMx->CCR2 = value; }          //set channel
  	if(chN == 3)      { TIMx->CCR3 = value; }          //set channel
  	if(chN == 4)      { TIMx->CCR4 = value; }          //set channel
	// REPEAT for CHn=2,  3, 4
	// REPEAT for CHn=2,  3, 4
	// REPEAT for CHn=2,  3, 4
