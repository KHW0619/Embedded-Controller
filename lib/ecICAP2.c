...
// 1. Initialize GPIO port and pin as AF
	...
	GPIO_ospeed(pinName, HIGH_SPEED);  						// speed HIGH_SPEED=3
...
// 2. Configure  IC number(user selected) with given IC pin(TIMx_CHn)
	switch(ICn){
			...
			case 3:
					...
					if (ICn==CHn) TIMx->CCMR2 |= TIM_CCMR2_CC3S_0;	    //01<<8   CC3S    Tx_Ch3=IC3
					else TIMx->CCMR2 |= TIM_CCMR2_CC3S_1;		     				//10<<8   CC3S    Tx_Ch4=IC3
					break;
			case 4:
...
// 3. Configure Activation Edge direction
	...
	switch(edge_type){
		case IC_RISE: TIMx->CCER |= (0b0000 << 4*(ICn - 1));	 break; //rising:  00
