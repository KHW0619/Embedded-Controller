...
void ADC_init(PinName_t pinName){  // trigmode 0 : SW, 1 : TRGO
	...
	GPIO_pupd(pinName, NO_PUPD);  			// EC_NONE = 0
	...
	ADC1->CR2 |= ADC_CR2_CONT;      // default : Continuous conversion mode (0: single / 1: conversion default)
	...
	ADC1->SQR3 |= (chN & ADC_SQR3_SQ1); 	// (chN & 0x1F)<< ADC_SQR3_SQ1 or (chN & 0x1F<<0)	// Choose the first channelID to sample
